...
import { ArrowLeft, Settings, Database, Mail, Bell } from "lucide-react-native";
...
const COLORS = {
  ...
  textLight: "#999999", bg: "#FFFFFF", cardBg: "#F9FAFB", border: "#E5E7EB", warning: "#F59E0B",
};
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.gradient2 }}>
      ...
      <LinearGradient colors={[COLORS.gradient1, COLORS.gradient2, COLORS.gradient3, COLORS.gradient4]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ paddingTop: Platform.OS === 'ios' ? sw(50) : sw(40), paddingBottom: sw(24), paddingHorizontal: sw(20) }}>
        ...
        <View style={{ alignItems: "center", marginBottom: sw(16) }}>
          <Text style={{ color: "#FFFFFF", fontSize: sw(16), fontWeight: "800", marginBottom: sw(5) }}>Settings</Text>
          <Text style={{ color: "rgba(255,255,255,0.95)", fontSize: sw(10) }}>System Configuration</Text>
        </View>
      </LinearGradient>
      <View style={{ flex: 1, backgroundColor: COLORS.bg, borderTopLeftRadius: sw(28), borderTopRightRadius: sw(28), marginTop: 0 }}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: sw(16), paddingTop: sw(16), paddingBottom: sw(120) }} showsVerticalScrollIndicator={false}>
          {[{ icon: Database, label: "Database" }, { icon: Mail, label: "Email Settings" }, { icon: Bell, label: "Notifications" }].map((item, idx) => (
            <TouchableOpacity key={idx} style={{ backgroundColor: COLORS.cardBg, borderRadius: sw(12), borderWidth: 1, borderColor: COLORS.border, padding: sw(12), marginBottom: sw(10), flexDirection: "row", alignItems: "center" }}>
              <View style={{ width: sw(36), height: sw(36), borderRadius: sw(10), backgroundColor: `${COLORS.primary}15`, alignItems: "center", justifyContent: "center", marginRight: sw(10) }}>
                <item.icon size={sw(16)} color={COLORS.primary} strokeWidth={2.5} />
              </View>
              <Text style={{ fontSize: sw(12), fontWeight: "700", color: COLORS.text, flex: 1 }}>{item.label}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
