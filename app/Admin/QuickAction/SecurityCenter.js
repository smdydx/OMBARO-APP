...
const COLORS = {
  ...
  textLight: "#999999", bg: "#FFFFFF", cardBg: "#F9FAFB", border: "#E5E7EB", danger: "#EF4444", success: "#10B981",
};
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.gradient2 }}>
      ...
      <LinearGradient colors={[COLORS.gradient1, COLORS.gradient2, COLORS.gradient3, COLORS.gradient4]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ paddingTop: Platform.OS === 'ios' ? sw(50) : sw(40), paddingBottom: sw(24), paddingHorizontal: sw(20) }}>
        ...
        <View style={{ alignItems: "center", marginBottom: sw(16) }}>
          <Text style={{ color: "#FFFFFF", fontSize: sw(16), fontWeight: "800", marginBottom: sw(5) }}>Security</Text>
          <Text style={{ color: "rgba(255,255,255,0.95)", fontSize: sw(10) }}>System Security</Text>
        </View>
      </LinearGradient>
      <View style={{ flex: 1, backgroundColor: COLORS.bg, borderTopLeftRadius: sw(28), borderTopRightRadius: sw(28), marginTop: 0 }}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: sw(16), paddingTop: sw(16), paddingBottom: sw(120) }} showsVerticalScrollIndicator={false}>
          <View style={{ backgroundColor: `${COLORS.success}15`, borderRadius: sw(12), borderWidth: 1, borderColor: COLORS.success, padding: sw(12), marginBottom: sw(12), flexDirection: "row", alignItems: "center" }}>
            <CheckCircle size={sw(20)} color={COLORS.success} strokeWidth={2.5} fill={COLORS.success} />
            <View style={{ marginLeft: sw(10), flex: 1 }}>
              <Text style={{ fontSize: sw(11), fontWeight: "700", color: COLORS.success }}>System Status: Secure</Text>
              <Text style={{ fontSize: sw(9), color: COLORS.success, opacity: 0.8 }}>All security measures active</Text>
            </View>
          </View>
          <View style={{ backgroundColor: COLORS.cardBg, borderRadius: sw(12), borderWidth: 1, borderColor: COLORS.border, padding: sw(12), marginBottom: sw(10) }}>
            <View style={{ flexDirection: "row", alignItems: "center", marginBottom: sw(6) }}>
              <Lock size={sw(14)} color={COLORS.primary} strokeWidth={2.5} />
              <Text style={{ fontSize: sw(11), fontWeight: "700", color: COLORS.text, marginLeft: sw(8) }}>Failed Logins: 3</Text>
            </View>
            <Text style={{ fontSize: sw(9), color: COLORS.textSecondary }}>2 blocked in last 24 hours</Text>
          </View>
        </ScrollView>
      </View>
    </View>
